...
package com.pixelcheateon.ui;

import com.pixelcheateon.Dungeon;
import com.pixelcheateon.PixelCheateon;
import com.pixelcheateon.items.ArcaneResin;
import com.pixelcheateon.items.Generator;
import com.pixelcheateon.items.Item;
import com.pixelcheateon.items.LiquidMetal;
import com.pixelcheateon.items.Recipe;
import com.pixelcheateon.items.bombs.Bomb;
import com.pixelcheateon.items.food.Blandfruit;
import com.pixelcheateon.items.food.Food;
import com.pixelcheateon.items.food.MeatPie;
import com.pixelcheateon.items.food.MysteryMeat;
import com.pixelcheateon.items.food.Pasty;
import com.pixelcheateon.items.food.StewedMeat;
import com.pixelcheateon.items.potions.AlchemicalCatalyst;
import com.pixelcheateon.items.potions.Potion;
import com.pixelcheateon.items.potions.brews.BlizzardBrew;
import com.pixelcheateon.items.potions.brews.CausticBrew;
import com.pixelcheateon.items.potions.brews.InfernalBrew;
import com.pixelcheateon.items.potions.brews.ShockingBrew;
import com.pixelcheateon.items.potions.elixirs.ElixirOfAquaticRejuvenation;
import com.pixelcheateon.items.potions.elixirs.ElixirOfArcaneArmor;
import com.pixelcheateon.items.potions.elixirs.ElixirOfDragonsBlood;
import com.pixelcheateon.items.potions.elixirs.ElixirOfHoneyedHealing;
import com.pixelcheateon.items.potions.elixirs.ElixirOfIcyTouch;
import com.pixelcheateon.items.potions.elixirs.ElixirOfMight;
import com.pixelcheateon.items.potions.elixirs.ElixirOfToxicEssence;
...
import com.pixelcheateon.items.spells.Alchemize;
import com.pixelcheateon.items.spells.AquaBlast;
import com.pixelcheateon.items.spells.ArcaneCatalyst;
import com.pixelcheateon.items.spells.BeaconOfReturning;
import com.pixelcheateon.items.spells.CurseInfusion;
import com.pixelcheateon.items.spells.FeatherFall;
import com.pixelcheateon.items.spells.MagicalInfusion;
import com.pixelcheateon.items.spells.PhaseShift;
import com.pixelcheateon.items.spells.ReclaimTrap;
import com.pixelcheateon.items.spells.Recycle;
import com.pixelcheateon.items.spells.SummonElemental;
import com.pixelcheateon.items.spells.TelekineticGrab;
import com.pixelcheateon.items.spells.WildEnergy;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					PixelCheateon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				PixelCheateon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) PixelCheateon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}

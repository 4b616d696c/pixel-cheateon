...
import com.pixelcheateon.PCAction;
...
import com.pixelcheateon.items.bags.Bag;
...
import com.pixelcheateon.windows.WndQuickBag;
import com.pixelcheateon.windows.WndUseItem;
...
public class Toolbar extends Component {
	...
	protected void createChildren() {
		...
		add(new Button(){
			...
			protected void onClick() {
				...
				if (Dungeon.hero.ready && !GameScene.cancel()) {
					...
					if (ControllerHandler.controllerActive){
						...
						info += Messages.get(WndKeyBindings.class, PCAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "quickslot_select") + "\n";
						info += Messages.get(WndKeyBindings.class, PCAction.RIGHT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "quickslot_assign") + "\n";
			...
			public GameAction keyAction() {
				if (btnWait.active) return PCAction.QUICKSLOT_SELECTOR;
				else				return null;
			}
		});
		...
		add(btnWait = new Tool(24, 0, 20, 26) {
			...
			@Override
			public GameAction keyAction() {
				return PCAction.WAIT;
			}

			@Override
			public GameAction secondaryTooltipAction() {
				return PCAction.WAIT_OR_PICKUP;
			}
		...
		add(new Button(){
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return PCAction.REST;
				else				return null;
			}
		});
		...
		add(new Button(){
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return PCAction.WAIT_OR_PICKUP;
				else				return null;
			}
		});
		...
		add(btnInventory = new Tool(0, 0, 24, 26) {
			...
			@Override
			public GameAction keyAction() {
				return PCAction.INVENTORY;
			}

			@Override
			public GameAction secondaryTooltipAction() {
				return PCAction.INVENTORY_SELECTOR;
			}
			...
			protected void createChildren() {
				...
				arrow.visible = PCSettings.interfaceSize() == 2;
		...
		add(new Button(){
			...
			protected void onClick() {
				if (Dungeon.hero.ready && !GameScene.cancel()) {
					...
					if (ControllerHandler.controllerActive){
						...
						info += Messages.get(WndKeyBindings.class, PCAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "container_select") + "\n";
					...
					Game.scene().addToFront(new RadialMenu(Messages.get(Toolbar.class, "container_prompt"), info, names, images){
						...
						public void onSelect(int idx, boolean alt) {
							...
							if (ControllerHandler.controllerActive){
								...
								info += Messages.get(WndKeyBindings.class, PCAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "item_select") + "\n";
								info += Messages.get(WndKeyBindings.class, PCAction.RIGHT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "item_use") + "\n";
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return PCAction.INVENTORY_SELECTOR;
				else				return null;
			}
		});
	...
	protected void layout() {
		...
		int startingSlot;
		if (PCSettings.quickSwapper() && quickslotsToShow < 6){
			quickslotsToShow = 3;
		...
		for(int i = startingSlot; i <= endingSlot; i++) {
			if (i == startingSlot && !PCSettings.flipToolbar() ||
				i == endingSlot && PCSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == startingSlot && PCSettings.flipToolbar() ||
					i == endingSlot && !PCSettings.flipToolbar()){
				btnQuick[i].border(2, 1);
				btnQuick[i].frame(86, 0, 20, 24);
			} else {
		...
		float shift = 0;
		switch(Mode.valueOf(PCSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);

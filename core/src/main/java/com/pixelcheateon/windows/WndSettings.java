...
import com.pixelcheateon.messages.Languages;
...
import com.pixelcheateon.ui.CheckBox;
import com.pixelcheateon.ui.GameLog;
import com.pixelcheateon.ui.Icons;
import com.pixelcheateon.ui.OptionSlider;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		PixelCheateon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
		...
		protected void createChildren() {
			...
			chkFullscreen = new CheckBox( Messages.get(this, "fullscreen") ) {
				...
				protected void onClick() {
					super.onClick();
					PCSettings.fullscreen(checked());
				}
			};
			if (DeviceCompat.supportsFullScreen()){
				chkFullscreen.checked(PCSettings.fullscreen());
			} else {
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							PixelCheateon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
				...
				chkSaver.checked( PCSettings.powerSaver() );
				add( chkSaver );
			}

			if (DeviceCompat.isAndroid()) {
				Boolean landscape = PCSettings.landscape();
				if (landscape == null){
				...
				btnOrientation = new RedButton(finalLandscape ?
					...
					@Override
					protected void onClick() {
						PCSettings.landscape(!finalLandscape);
					}
				};
				add(btnOrientation);
			}
			...
			optBrightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				protected void onChange() {
					PCSettings.brightness(getSelectedValue());
				}
			};
			optBrightness.setSelectedValue(PCSettings.brightness());
			add(optBrightness);
			...
			optVisGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					PCSettings.visualGrid(getSelectedValue());
				}
			};
			optVisGrid.setSelectedValue(PCSettings.visualGrid());
			add(optVisGrid);
			...
			optFollowIntensity = new OptionSlider(Messages.get(this, "camera_follow"),
				...
				@Override
				protected void onChange() {
					PCSettings.cameraFollow(getSelectedValue());
				}
			};
			optFollowIntensity.setSelectedValue(PCSettings.cameraFollow());
		...
		protected void createChildren() {
			...
			if (Math.min(wMin, hMin) >= 2*Game.density){
				optUIMode = new OptionSlider(
					...
					@Override
					protected void onChange() {
						PCSettings.interfaceSize(getSelectedValue());
						PixelCheateon.seamlessResetScene();
					}
				};
				optUIMode.setSelectedValue(PCSettings.interfaceSize());
				add(optUIMode);
			}
			...
			if ((int)Math.ceil(2* Game.density) < PixelScene.maxDefaultZoom) {
				optUIScale = new OptionSlider(Messages.get(this, "scale"),
					...
					@Override
					protected void onChange() {
						if (getSelectedValue() != PCSettings.scale()) {
							PCSettings.scale(getSelectedValue());
							PixelCheateon.seamlessResetScene();
						}
					}
				};
			...
			if (PCSettings.interfaceSize() == 0) {
				btnToolbarSettings = new RedButton(Messages.get(this, "toolbar_settings"), 9){
					@Override
					protected void onClick() {
						PixelCheateon.scene().addToFront(new Window(){
...

							{
								...
								btnSplit = new RedButton(Messages.get(WndSettings.UITab.this, "split")) {
									...
									protected void onClick() {
										...
										PCSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
										Toolbar.updateLayout();
									}
								};
								if (PCSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name())) {
								...
								btnGrouped = new RedButton(Messages.get(WndSettings.UITab.this, "group")) {
									...
									protected void onClick() {
										...
										textColor(TITLE_COLOR);
										btnCentered.textColor(WHITE);
										PCSettings.toolbarMode(Toolbar.Mode.GROUP.name());
										Toolbar.updateLayout();
									}
								};
								if (PCSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name())) {
								...
								btnCentered = new RedButton(Messages.get(WndSettings.UITab.this, "center")) {
									...
									protected void onClick() {
										...
										btnGrouped.textColor(WHITE);
										textColor(TITLE_COLOR);
										PCSettings.toolbarMode(Toolbar.Mode.CENTER.name());
										Toolbar.updateLayout();
									}
								};
								if (PCSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name())) {
								...
								chkQuickSwapper = new CheckBox(Messages.get(WndSettings.UITab.this, "quickslot_swapper")) {
									...
									protected void onClick() {
										super.onClick();
										PCSettings.quickSwapper(checked());
										Toolbar.updateLayout();
									}
								};
								chkQuickSwapper.checked(PCSettings.quickSwapper());
								add(chkQuickSwapper);
								...
								chkFlipToolbar = new CheckBox(Messages.get(WndSettings.UITab.this, "flip_toolbar")) {
									...
									protected void onClick() {
										super.onClick();
										PCSettings.flipToolbar(checked());
										Toolbar.updateLayout();
									}
								};
								chkFlipToolbar.checked(PCSettings.flipToolbar());
								add(chkFlipToolbar);
								...
								chkFlipTags = new CheckBox(Messages.get(WndSettings.UITab.this, "flip_indicators")){
									...
									protected void onClick() {
										super.onClick();
										PCSettings.flipTags(checked());
										GameScene.layoutTags();
									}
								};
								chkFlipTags.checked(PCSettings.flipTags());
								add(chkFlipTags);
			...
			chkFont.checked(PCSettings.systemFont());
			add(chkFont);
		}
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						PixelCheateon.scene().addToFront(new WndKeyBindings(false));
					}
				};
			...
			if (ControllerHandler.isControllerConnected()){
				btnControllerBindings = new RedButton(Messages.get(this, "controller_bindings")){
					...
					protected void onClick() {
						super.onClick();
						PixelCheateon.scene().addToFront(new WndKeyBindings(true));
					}
				};
			...
			optControlSens = new OptionSlider(
				...
				@Override
				protected void onChange() {
					PCSettings.controllerPointerSensitivity(getSelectedValue());
				}
			};
			optControlSens.setSelectedValue(PCSettings.controllerPointerSensitivity());
			add(optControlSens);
			...
			optHoldMoveSens = new OptionSlider(
				...
				@Override
				protected void onChange() {
					PCSettings.movementHoldSensitivity(getSelectedValue());
				}
			};
			optHoldMoveSens.setSelectedValue(PCSettings.movementHoldSensitivity());
			add(optHoldMoveSens);
		}
	...
	private static class DataTab extends Component{
		...
		protected void createChildren() {
			...
			chkNews = new CheckBox(Messages.get(this, "news")){
				...
				protected void onClick() {
					super.onClick();
					PCSettings.news(checked());
					News.clearArticles();
				}
			};
			chkNews.checked(PCSettings.news());
			add(chkNews);
			...
			if (Updates.supportsUpdates() && Updates.isUpdateable()) {
				chkUpdates = new CheckBox(Messages.get(this, "updates")) {
					...
					protected void onClick() {
						super.onClick();
						PCSettings.updates(checked());
						Updates.clearUpdate();
					}
				};
				chkUpdates.checked(PCSettings.updates());
				add(chkUpdates);
				...
				if (Updates.supportsBetaChannel()){
					...
					chkBetas.checked(PCSettings.betas());
					add(chkBetas);
				}
			}
			...
			if (!DeviceCompat.isDesktop()){
				chkWifi = new CheckBox(Messages.get(this, "wifi")){
					...
					protected void onClick() {
						super.onClick();
						PCSettings.WiFi(checked());
					}
				};
				chkWifi.checked(PCSettings.WiFi());
				add(chkWifi);
			}
		}
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					PCSettings.musicVol(getSelectedValue());
				}
			};
			optMusic.setSelectedValue(PCSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					PCSettings.music(!checked());
				}
			};
			chkMusicMute.checked(!PCSettings.music());
			add(chkMusicMute);
			...
			optSFX = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					PCSettings.SFXVol(getSelectedValue());
					if (Random.Int(100) == 0){
			...
			optSFX.setSelectedValue(PCSettings.SFXVol());
			add(optSFX);
			...
			chkMuteSFX = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					PCSettings.soundFx(!checked());
					Sample.INSTANCE.play( Assets.Sounds.CLICK );
				}
			};
			chkMuteSFX.checked(!PCSettings.soundFx());
			add( chkMuteSFX );
			...
			if (DeviceCompat.isiOS()){
				...
				chkIgnoreSilent = new CheckBox( Messages.get(this, "ignore_silent") ){
					...
					protected void onClick() {
						super.onClick();
						PCSettings.ignoreSilentMode(checked());
					}
				};
				chkIgnoreSilent.checked(PCSettings.ignoreSilentMode());
				add(chkIgnoreSilent);
				...
				chkMusicBG = new CheckBox( Messages.get(this, "music_bg") ){
					...
					protected void onClick() {
						super.onClick();
						PCSettings.playMusicInBackground(checked());
					}
				};
				chkMusicBG.checked(PCSettings.playMusicInBackground());
				add(chkMusicBG);
			}
		}
				...
				RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName()), 7){
					...
					protected void onClick() {
						...
						PixelCheateon.seamlessResetScene(new Game.SceneChangeCallback() {
							@Override
							public void beforeCreate() {
								PCSettings.language(langs.get(langIndex));
								GameLog.wipe();
								Game.platform.resetGenerators();
							}
							@Override
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						PixelCheateon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}

...
public enum Rankings {
	...
	public void submit( boolean win, Class cause ) {
		...
		Matcher m = p.matcher(PixelCheateon.version);
		if (m.find()) {
	...
	public int calculateScore(){
		...
		if (Dungeon.initialVersion > PixelCheateon.v1_2_3){
	...
	public void load() {
		...
		try {
			...
			if (bundle.contains(LATEST_DAILY)){
				...
				if (latestDate > PCSettings.lastDaily()){
					PCSettings.lastDaily(latestDate);
				}
			}

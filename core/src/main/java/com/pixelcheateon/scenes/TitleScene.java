...
import com.pixelcheateon.effects.Fireball;
...
import com.pixelcheateon.services.updates.AvailableUpdateData;
...
import com.pixelcheateon.windows.WndSettings;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				PixelCheateon.switchNoFade( RankingsScene.class );
			}
		};
		...
		StyledButton btnBadges = new StyledButton(GREY_TR, Messages.get(this, "badges")){
			@Override
			protected void onClick() {
				PixelCheateon.switchNoFade( BadgesScene.class );
			}
		};
		...
		StyledButton btnAbout = new StyledButton(GREY_TR, Messages.get(this, "about")){
			@Override
			protected void onClick() {
				PixelCheateon.switchScene( AboutScene.class );
			}
		};
	...
	private static class NewsButton extends StyledButton {
		...
		public NewsButton(Chrome.Type type, String label ){
			super(type, label);
			if (PCSettings.news()) News.checkForNews();
		}
		...
		public void update() {
			...
			if (unreadCount == -1 && News.articlesAvailable()){
				long lastRead = PCSettings.newsLastRead();
				if (lastRead == 0){
					if (News.articles().get(0) != null) {
						PCSettings.newsLastRead(News.articles().get(0).date.getTime());
					}
				} else {
					unreadCount = News.unreadArticles(new Date(PCSettings.newsLastRead()));
					if (unreadCount > 0) {
		...
		protected void onClick() {
			super.onClick();
			PixelCheateon.switchNoFade( NewsScene.class );
		}
	}
	...
	private static class ChangesButton extends StyledButton {
		...
		public ChangesButton( Chrome.Type type, String label ){
			super(type, label);
			if (PCSettings.updates()) Updates.checkForUpdate();
		}
				...
				PixelCheateon.scene().addToFront(new WndOptions(
					...
					protected void onSelect(int index) {
						if (index == 0) {
							...
							PixelCheateon.switchNoFade( ChangesScene.class );
						}
					}
				});
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			PixelCheateon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			PixelCheateon.switchNoFade(SupporterScene.class);
		}
	}
}

...
import com.pixelcheateon.services.news.NewsArticle;
...
import com.pixelcheateon.windows.WndTitledMessage;
...
public class NewsScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnSite = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "read_more")){
			...
			protected void onClick() {
				...
				PixelCheateon.platform.openURI(link);
			}
		};
	...
	private static class NewsInfo extends Component {
		...
		protected void createChildren() {
			...
			if (!News.articlesAvailable()){
				if (PCSettings.news()) {
					if (PCSettings.WiFi() && !Game.platform.connectedToUnmeteredNetwork()) {
	...
	private static class ArticleButton extends StyledButton {
		...
		protected void onClick() {
			...
			if (article.date.getTime() > PCSettings.newsLastRead()){
				PCSettings.newsLastRead(article.date.getTime());
			}
			PixelCheateon.scene().addToFront(new WndArticle(article));
		}
	}

...
import com.pixelcheateon.windows.WndError;
import com.pixelcheateon.windows.WndHardNotification;
...
public class WelcomeScene extends PixelScene {
	...
	private static final int LATEST_UPDATE = PixelCheateon.v2_0_0;
	...
	public void create() {
		...
		final int previousVersion = PCSettings.version();
		...
		if (!triedCleaningTemp && FileUtils.cleanTempFiles()){
			add(new WndHardNotification(Icons.get(Icons.WARNING),
				...
				public void hide() {
					...
					PixelCheateon.resetScene();
				}
			});
			return;
		}

		if (PixelCheateon.versionCode == previousVersion && !PCSettings.intro()) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				super.onClick();
				if (previousVersion == 0 || PCSettings.intro()){

					if (previousVersion > 0){
						updateVersion(previousVersion);
					}

					PCSettings.version(PixelCheateon.versionCode);
		...
		if (previousVersion != 0 && !PCSettings.intro()){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					PixelCheateon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= PixelCheateon.versionCode) {
		...
		if (PCSettings.intro() && ControllerHandler.isControllerConnected()){
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion <= PixelCheateon.v2_0_0){
			Badges.loadGlobal();

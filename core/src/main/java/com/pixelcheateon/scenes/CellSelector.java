...
public class CellSelector extends ScrollArea {
	...
	private float zoom( float value ) {
		...
		PCSettings.zoom((int) (value - PixelScene.defaultZoom));
		camera.zoom( value );
	...
	//used for movement
	private GameAction heldAction1 = PCAction.NONE;
	private GameAction heldAction2 = PCAction.NONE;
	//not used for movement, but helpful if the player holds 3 keys briefly
	private GameAction heldAction3 = PCAction.NONE;

	private float heldDelay = 0f;
	private boolean delayingForRelease = false;

	private static float initialDelay(){
		switch (PCSettings.movementHoldSensitivity()){
			case 0:
	...
	private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
		...
		public boolean onSignal(KeyEvent event) {
			...
			if (!event.pressed){
				...
				if (action == PCAction.ZOOM_IN){
					zoom( camera.zoom+1 );
					mouseZoom = camera.zoom;
					return true;

				} else if (action == PCAction.ZOOM_OUT){
					zoom( camera.zoom-1 );
					mouseZoom = camera.zoom;
					return true;
				}
				
				if (heldAction1 != PCAction.NONE && heldAction1 == action) {
					heldAction1 = PCAction.NONE;
					if (heldAction2 != PCAction.NONE){
						heldAction1 = heldAction2;
						heldAction2 = PCAction.NONE;
						if (heldAction3 != PCAction.NONE){
							heldAction2 = heldAction3;
							heldAction3 = PCAction.NONE;
						}
					}
				} else if (heldAction2 != PCAction.NONE && heldAction2 == action){
					heldAction2 = PCAction.NONE;
					if (heldAction3 != PCAction.NONE){
						heldAction2 = heldAction3;
						heldAction3 = PCAction.NONE;
					}
				} else if (heldAction3 != PCAction.NONE && heldAction3 == action){
					heldAction3 = PCAction.NONE;
				}
				...
				lastCellMoved = -1;
				if (heldAction1 == PCAction.NONE){
					heldAction1 = action;
					heldDelay = initialDelay();
					delayingForRelease = false;
				} else if (heldAction2 == PCAction.NONE){
					heldAction2 = action;
				} else {
					heldAction3 = action;
				}
	...
	private GameAction leftStickAction = PCAction.NONE;
	...
	public void update() {
		...
		if (newLeftStick != leftStickAction){
			if (leftStickAction == PCAction.NONE){
				heldDelay = initialDelay();
				Dungeon.hero.resting = false;
			} else if (newLeftStick == PCAction.NONE && heldDelay > 0f){
				heldDelay = 0f;
		...
		if ((heldAction1 != PCAction.NONE || leftStickAction != PCAction.NONE) && Dungeon.hero.ready){
			processKeyHold();
	...
	private int directionFromAction(GameAction action){
		if (action == PCAction.N)  return -Dungeon.level.width();
		if (action == PCAction.NE) return +1-Dungeon.level.width();
		if (action == PCAction.E)  return +1;
		if (action == PCAction.SE) return +1+Dungeon.level.width();
		if (action == PCAction.S)  return +Dungeon.level.width();
		if (action == PCAction.SW) return -1+Dungeon.level.width();
		if (action == PCAction.W)  return -1;
		if (action == PCAction.NW) return -1-Dungeon.level.width();
		else                        return 0;
	}
	...
	public void resetKeyHold(){
		heldAction1 = heldAction2 = heldAction3 = PCAction.NONE;
	}

...
import com.pixelcheateon.actors.hero.HeroClass;
...
import com.pixelcheateon.ui.ExitButton;
import com.pixelcheateon.ui.IconButton;
...
import com.pixelcheateon.windows.WndTextInput;
import com.pixelcheateon.utils.DungeonSeed;
import com.pixelcheateon.windows.WndChallenges;
import com.pixelcheateon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				PixelCheateon.scene().addToFront(w);
			}
		...
		if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
			...
			PCSettings.challenges(0);
			PCSettings.customSeed("");
		}
		...
		add( btnExit );
		btnExit.visible = btnExit.active = !PCSettings.intro();
	...
	private void setSelectedHero(HeroClass cl){
		...
		if (landscape()) {
			...
			btnOptions.visible = btnOptions.active = !PCSettings.intro();
	...
	protected void onBackPressed() {
		if (btnExit.active){
			PixelCheateon.switchScene(TitleScene.class);
		} else {
			super.onBackPressed();
		}
	}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				PixelCheateon.scene().addToFront( new WndMessage(cl.unlockMsg()));
		...
		protected void createChildren() {
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
					@Override
					protected void onClick() {
						String existingSeedtext = PCSettings.customSeed();
						PixelCheateon.scene().addToFront(new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
								...
								if (positive && seed != -1){
									...
									for (GamesInProgress.Info info : GamesInProgress.checkAll()){
										if (info.customSeed.isEmpty() && info.seed == seed){
											PCSettings.customSeed("");
											icon.resetColor();
											PixelCheateon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
											return;
										}
									}

									PCSettings.customSeed(text);
				...
				if (!PCSettings.customSeed().isEmpty()) seedButton.icon().hardlight(1f, 1.5f, 0.67f);;
				...
				StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
					...
					protected void onClick() {
						...
						long diff = (PCSettings.lastDaily() + DAY) - Game.realTime;
						if (diff > 24*HOUR){
							PixelCheateon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
							return;
						}

						for (GamesInProgress.Info game : GamesInProgress.checkAll()){
							if (game.daily){
								PixelCheateon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
								return;
							}
						}
						...
						PixelCheateon.scene().addToFront(new WndOptions(
								icon,
				...
				challengeButton.icon(Icons.get(PCSettings.challenges() > 0 ? Icons.CHALLENGE_ON : Icons.CHALLENGE_OFF));
				add(challengeButton);

...
import com.pixelcheateon.ui.Tooltip;
...
public class PixelScene extends Scene {
	...
	public void create() {
		...
		Game.platform.setupFontGenerators(renderedTextPageSize, PCSettings.systemFont());
	...
	public void update() {
		...
		if (DeviceCompat.isDesktop() && fullscreenListener == null){
			KeyEvent.addKeyListener(fullscreenListener = new Signal.Listener<KeyEvent>() {
				...
				public boolean onSignal(KeyEvent keyEvent) {
					...
					if (alt && enter){
						PCSettings.fullscreen(!PCSettings.fullscreen());
						return true;
					}
		...
		if (!Cursor.isCursorCaptured()) {
			if (Math.abs(ControllerHandler.rightStickPosition.x) >= 0.2f
				...
				int sensitivity = PCSettings.controllerPointerSensitivity() * 100;
	...
	public static boolean landscape(){
		return PCSettings.interfaceSize() > 0 || Game.width > Game.height;
	}

...
package com.pixelcheateon.journal.services.news;
...
import com.pixelcheateon.services.news.NewsService;
...
public class News {
	...
	public static void checkForNews(){
		...
		service.checkForArticles(!PCSettings.WiFi(), useHTTPS, new NewsService.NewsResultCallback() {
			@Override
	...
	public static Image parseArticleIcon(NewsArticle article){
		...
		//if we run into any formatting errors (or icon is null), default to the news icon
			...
			if (article.icon != null) PixelCheateon.reportException(e);
